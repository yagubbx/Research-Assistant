...
\textbf{Injected incident: arXiv raises a provider error}\vspace{0.4cm}
\begin{itemize}\setlength\itemsep{0.35cm}
\item Wikipedia and web evidence still reaches synthesis.
\item References include an explicit arXiv-unavailable note.
\item Transient HTTP failures retry; 401 does not.
\item All sources unavailable: clear error, no invented answer.
\end{itemize}\vspace{0.3cm}
\small Defaults: 3 attempts; 0.5 / 1.0 s backoff; 20 s source deadline; 60 s synthesis deadline.
\end{frame}
...
\small Live demo requires configured API keys. Docker build, offline run and UI health checks passed.
\end{frame}

\begin{frame}{Testing evidence}
\begin{columns}[T,onlytextwidth]
\column{.46\textwidth}{\Huge\color{accent}114} passing tests\\[0.45cm]
{\Huge\color{accent}93.91\%} application coverage\\[0.35cm]
...
\end{columns}\vspace{0.5cm}
\small Ruff passes; mypy reports no issues in 17 files. Tests use mocks and deny live sockets.
\end{frame}
...
\small Docker acceptance passed. Live Gemini and hosted CI remain separate checks. Bonus credit is assessed by the instructor.
\end{frame}
