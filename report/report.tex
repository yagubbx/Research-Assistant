...
Engineering verification: 16 September 2026\\
...
The verification run passed 114 offline tests with 93.91\% application coverage. A five-question retrieval benchmark measured a median 2.412 seconds sequentially and 0.802 seconds concurrently, a 3.01-fold speedup. These are explicitly simulated HTTP timings with caches bypassed; they do not establish live provider latency or research quality.

The package includes source, tests, Docker configuration, this report, the defense slides and run artefacts. Docker build, offline CLI and container UI health checks passed; live LLM execution remains unverified without a user key. The repository address was supplied by the team; publication, review history, final tag and signed contributions require genuine team actions. Substantial AI assistance is disclosed in Section 9. This report records engineering evidence rather than certifying a grade or submission status.
...
Anthropic and Tavily are the configured defaults, matching the supplied module. OpenAI, Gemini and alternative web providers remain available through environment configuration. This is a compatibility choice, not an empirical quality or cost comparison. Live credentials were unavailable during verification. Application business logic never constructs provider SDK clients. All source fetching uses the three public \texttt{ai.fetch\_*} functions, and synthesis calls \texttt{ai.synthesize} in an isolated process.
...
When every source fails or returns empty evidence, the application refuses synthesis. This avoids producing an answer with invented support. Invalid model output is also rejected: an empty answer, no valid citation, or a factual sentence without a citation produces a clear error. Structural validation cannot prove that the cited passage supports a claim; that remains a stated quality limitation.
...
The measured suite contains 114 passing tests, including all 16 unchanged course smoke tests. Coverage of the application package is 93.91\%, above the required 60\% gate. The course provider implementation is excluded from the application coverage denominator and separately exercised by its smoke tests. Raw coverage data is supplied in \texttt{artefacts/coverage.json}. Ruff passed after formatting; mypy reported no issues in 17 application files.
...
\noindent\begin{tabularx}{\linewidth}{@{}p{2.9cm}>{\raggedright\arraybackslash}X@{}}\toprule
...
The package is prepared for the team-specified GitHub repository. Genuine reviews, member commit history, a pushed final tag and Moodle delivery cannot be substituted by generated files. The contribution statement records the known team identities and leaves attestations unsigned. The team must fill its actual ownership and review history before signing; the package does not manufacture commit percentages or signatures.
...
\item \textbf{Operational verification.} Docker build/run and UI health passed. Hosted CI status, protected branches, member reviews and submission were not observed. The 338,510,564-byte image exceeds the optional 250 MB target.
...
OpenAI Codex substantially drafted the application modules, tests, configuration, documentation and presentation in this package, and executed the local engineering checks. The course supplied the unchanged AI package, sample questions, smoke tests and document templates. The team supplied its names and repository address. This disclosure does not claim that team members have already reviewed or can defend every generated line; that must be established through their own review and oral-defense preparation.
